...
import { createCorsMiddleware } from "./lib/http.js";
...
function createServer() {
  const presence = env.REDIS_URL
    ? new RedisPresence(env.REDIS_URL)
    : new LocalPresence();
  const driver = env.REDIS_URL
    ? new RedisDriver(env.REDIS_URL)
    : new LocalDriver();
...
async function reservePublicSeat(oneTimeToken: string) {
  ...
  const room =
    rooms
      ...
      .sort((left, right) => {
        const leftAnchor = Number(
          left.metadata?.anchorMmr ?? ticket.profile.mmr,
        );
        const rightAnchor = Number(
          right.metadata?.anchorMmr ?? ticket.profile.mmr,
        );
...
async function joinPrivateSeat(oneTimeToken: string, requestedCode: string) {
  ...
  const room = rooms.find(
    (entry) =>
      String(entry.metadata?.roomCode ?? "").toUpperCase() === roomCode,
  );
...
function configureHttp(app: express.Application) {
  ...
  app.post(
    "/internal/tickets/public",
    async (request: express.Request, response: express.Response) => {
      try {
        response.json(
          await reservePublicSeat(String(request.body?.oneTimeToken ?? "")),
        );
      } catch (error) {
        response.status(400).json({
          error:
            error instanceof Error
              ? error.message
              : "Failed to create public ticket.",
        });
      }
    },
  );
  ...
  app.post(
    ...
    async (request: express.Request, response: express.Response) => {
      try {
        response.json(
          await reservePrivateSeat(String(request.body?.oneTimeToken ?? "")),
        );
      } catch (error) {
        response.status(400).json({
          error:
            error instanceof Error
              ? error.message
              : "Failed to create private ticket.",
        });
      }
    },
  );
  ...
  app.post(
    ...
    async (request: express.Request, response: express.Response) => {
      try {
        ...
        response.json(
          await joinPrivateSeat(
            String(request.body?.oneTimeToken ?? ""),
            parsed.roomCode,
          ),
        );
      } catch (error) {
